...
const tenantService = {
  ...
  // Exportera hyresgäster som SQL
  exportToSql: async () => {
    try {
      const response = await api.get('/api/tenants/export-sql', {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', 'tenants_export.sql');
      ...
      console.error('Error exporting tenants to SQL:', error);
      throw error;
    }
  }
};
