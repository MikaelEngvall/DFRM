import React from 'react';

const FLAGS = {
  sv: (
    <svg className="w-5 h-5" viewBox="0 0 512 512">
      <path fill="#006AA7" d="M0 0h512v512H0z"/>
      <path fill="#FECC00" d="M128 0h96v512h-96zM0 208h512v96H0z"/>
    </svg>
  ),
  en: (
    <svg className="w-5 h-5" viewBox="0 0 512 512">
      <path fill="#012169" d="M0 0h512v512H0z"/>
      <path fill="#FFF" d="M512 0v64L322 256l190 187v69h-67L254 324 68 512H0v-68l186-187L0 74V0h62l192 188L440 0z"/>
      <path fill="#C8102E" d="M184 324l11 34L42 512H0v-3l184-185zm124-12l54 8 150 147v45L308 312zM512 0L320 196l-4-44L466 0h46zM0 1l193 189-59-8L0 49V1z"/>
      <path fill="#FFF" d="M176 0v512h160V0H176zM0 176v160h512V176H0z"/>
      <path fill="#C8102E" d="M0 208v96h512v-96H0zM208 0v512h96V0h-96z"/>
    </svg>
  ),
  pl: (
    <svg className="w-5 h-5" viewBox="0 0 512 512">
      <path fill="#FFF" d="M0 0h512v512H0z"/>
      <path fill="#DC143C" d="M0 256h512v256H0z"/>
    </svg>
  ),
  uk: (
    <svg className="w-5 h-5" viewBox="0 0 512 512">
      <path fill="#005BBB" d="M0 0h512v512H0z"/>
      <path fill="#FFD500" d="M0 256h512v256H0z"/>
    </svg>
  ),
};

const FlagIcon = ({ code, className = '' }) => {
  return (
    <span className={`inline-block ${className}`} role="img" aria-label={`Flag for ${code}`}>
      {FLAGS[code] || null}
    </span>
  );
};

export default FlagIcon; 